...
func gerar_catalogo(colecao:Dictionary, visualizacao_de_colecao:bool = false) -> void:
	...
	recalcular_paginas(visualizacao_de_colecao)
	
	await pagina_calculada
	...
	for edicao in DATA.CardInfo:
		...
		for idx in DATA.CardInfo[edicao]:
			...
			catalogo_de_cartas[edicao][int(idx)]["pagina"] = arredondar(quantidade_de_cartas, quantidade_de_maxima_de_cartas_por_paginas)
		...
		for edicao in catalogo_de_cartas:
			for idx in catalogo_de_cartas[edicao]:
				if catalogo_de_cartas[edicao][idx]["raca"] == raca_do_deck:
					if int(pagina_atual) == int(catalogo_de_cartas[edicao][idx]["pagina"]):
...
func recalcular_paginas(visualizacao_de_colecao:bool) -> void:
	var quantidade_de_cartas_temp: int = 0
	if !visualizacao_de_colecao:
		for edicao in catalogo_de_cartas:
			for idx in catalogo_de_cartas[edicao]:
				if catalogo_de_cartas[edicao][idx]["raca"] == raca_do_deck:
					quantidade_de_cartas_temp += 1
	else:
		for edicao in catalogo_de_cartas:
			for idx in catalogo_de_cartas[edicao]:
				quantidade_de_cartas_temp += 1
	
	quantidade_de_cartas = quantidade_de_cartas_temp
